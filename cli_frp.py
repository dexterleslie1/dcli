...
class FrpCli(object):
    ...
    def install(self):
        ...
        # 询问用户是否安装frp服务器或者客户端
        var_install_frp_server = raw_input("是否安装frp服务器? [y/n]: ")
        var_install_frp_client = "n"
        if var_install_frp_server.lower() != "y":
            var_install_frp_client = raw_input("是否安装frp客户端? [y/n]: ")
        ...
        if var_install_frp_server.lower() == "y" or var_install_frp_client.lower() == "y":
            ...
            if var_install_frp_client.lower() == "y":
                var_frp_server_ip = raw_input("输入frp服务器ip地址（NOTE：不提供则使用默认配置的）: ") or ""
            var_frp_server_port = raw_input("输入frp服务器端口（NOTE：不提供则使用默认配置的）: ") or "0"
            var_frp_privilege_token = raw_input("输入frp服务器身份鉴别token（NOTE：不提供则使用默认配置的）: ") or ""
